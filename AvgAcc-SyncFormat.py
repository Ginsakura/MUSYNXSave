...
def get_db_connection() -> sql.Connection:
	return sql.connect('./musync_data/HitDelayHistory.db');

def ReadData() -> None:
	with open('./musync_data/Acc-Sync.json','r') as f:
		data=f.readlines();
	for line in data:
		parts = line.split(',');  # [acc, sync]
		acc_sync[float(parts[0])] = [float(parts[1])];  # {acc: sync}

def SQLSelect() -> None:
	with get_db_connection() as db:
		cur:sql.Cursor = db.cursor();
		cur.execute("select * from HitDelayHistory");
		data = cur.fetchall();
		for ids in data:
			DataFormat(ids);


def DataFormat(data) -> None:
	acc = round(data[3],4);
	if acc in acc_sync:

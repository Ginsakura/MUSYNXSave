...
from . import Config, Logger, Toolkit

class AllHitAnalyze(object):
    """docstring for HitAnalyze"""
    # 阈值常量（乘以10000）
    _THRESHOLD_5   = 5 * 10000      # 50000
    _THRESHOLD_10  = 10 * 10000     # 100000
    _THRESHOLD_20  = 20 * 10000     # 200000
    _THRESHOLD_45  = 45 * 10000     # 450000
    _THRESHOLD_90  = 90 * 10000     # 900000
    _THRESHOLD_150 = 150 * 10000    # 1500000
    _THRESHOLD_250 = 250 * 10000    # 2500000

    _db_mode: bool = False
    _sum_y_num: int = 0          # with miss
    _sum_y_num_ext: int = 0      # only Exact
    _sum_y_num_core: int = 0     # only Cyan Exact
    # Cyan_Exact,Exact,Great,Right,Miss
    _rate: list[int] = [0,0,0,0,0]
    # ±5ms,±10ms,±20ms,±45ms,Exact,Great,Right,Miss
    _accurate_rate: list[int] = [0,0,0,0,0,0,0,0]
    
    _x_axis: list[int] = [i for i in range(-150,251)]
    _y_axis: list[int] = []

    _pdf_all: list[float] = []
    _pdf_ext: list[float] = []
    _pdf_core: list[float] = []

    def __init__(self, root: tk.Tk | tk.Toplevel, list_data: list[int] | None = None, bytes_data: bytes | None = None):
        # super(AllHitAnalyze, self).__init__()
        self._root: tk.Tk | tk.Toplevel = root
        self._logger: logging.Logger = Logger.GetLogger(name="AllHitAnalyze")
        self._bat_chart_window: tk.Toplevel = tk.Toplevel(self._root)
        ...
        if list_data is None:
            # 否则使用 bytes_data 进行分析, 如果 bytes_data 也为 None, 则从数据库加载数据进行分析
            if bytes_data is not None:
                res: list[tuple[bytes]] = [(bytes_data,)]  # 模拟数据库查询结果, 单行数据
            else:
            ...
            for row in res:
                ...
                self._analyze_data(ints, hit_map_a, hit_map_b)
        else:
            self._analyze_data(list_data, hit_map_a, hit_map_b)

        self._y_axis = hit_map_a + hit_map_b
        ...
        # 计算三条曲线的数据
        self._pdf_all = Toolkit._calculate_pdf( self._x_axis, self._avg,      self._var,      self._std,      self._sum_y_num     )
        self._pdf_ext = Toolkit._calculate_pdf( self._x_axis, self._avg_ext,  self._var_ext,  self._std_ext,  self._sum_y_num_ext )
        self._pdf_core = Toolkit._calculate_pdf(self._x_axis, self._avg_core, self._var_core, self._std_core, self._sum_y_num_core)
        
    def _analyze_data(self, ints: tuple[int, ...] | list[int], hit_map_a: list[int], hit_map_b: list[int]) -> None:
        for ival in ints:
            self._sum_y_num += 1
            abs_ival: int = abs(ival)
            # 分类统计（基于 abs_ival）
            if abs_ival < self._THRESHOLD_5:
                self._rate[0] += 1
                self._accurate_rate[0] += 1
            elif abs_ival < self._THRESHOLD_10:
                self._rate[0] += 1
                self._accurate_rate[1] += 1
            elif abs_ival < self._THRESHOLD_20:
                self._rate[0] += 1
                self._accurate_rate[2] += 1
            elif abs_ival < self._THRESHOLD_45:
                self._rate[0] += 1
                self._accurate_rate[3] += 1
            elif abs_ival < self._THRESHOLD_90:
                self._rate[1] += 1
                self._accurate_rate[4] += 1
            elif abs_ival < self._THRESHOLD_150:
                self._rate[2] += 1
                self._accurate_rate[5] += 1
            elif abs_ival < self._THRESHOLD_250:
                self._rate[3] += 1
            ...
            # 统计 exact / cyan exact
            if abs_ival < self._THRESHOLD_45:
    ...
    def show(self) -> tuple[list[int], int]:
        """主绘制入口"""
        # 预设中文字体 (如果你系统安装了此字体)
        plot.rcParams['font.serif'] = ["LXGW WenKai Mono"]
        plot.rcParams["font.sans-serif"] = ["LXGW WenKai Mono"]

        title: str = f"{'All Hit Analyze' if self._db_mode else 'One Map Analyze'} "\
        ...
        # 创建画板
        fig: Figure = plot.figure(title, figsize=(16, 9))
        fig.clear()
    ...
    def _draw_histogram_and_curves(self, fig: Figure, ax: Axes) -> None:
        ...
        # 3. 绘制曲线, 但先保存对象引用以便后续控制 (初始状态设为隐藏 visible=False)
        line_all, = ax.plot(self._x_axis, self._pdf_all, c='grey', lw=2,
                             label=f'Global Fit (μ={self._avg:.2f}, σ={self._std:.2f})')
        line_ext, = ax.plot(self._x_axis, self._pdf_ext, c='black', lw=2,
                             label=f'Exact Fit (μ={self._avg_ext:.2f}, σ={self._std_ext:.2f})')
        line_core, = ax.plot(self._x_axis, self._pdf_core, c='blue', lw=2,

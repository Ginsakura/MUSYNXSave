...
namespace BMSLib
{
	...
	public static class LocalUdpClient
	{
		...
		/// <exception cref="SecurityException">目标地址非本机</exception>
		...
		/// <summary>
		/// 序列化并发送复杂数据（String, Int, List<Float>）
		/// </summary>
		public static void SendStructuredData(string text, int number, List<float> floats)
		{
			byte[] serialized = SerializeData(text, number, floats);
			Send(serialized);
		}
	...
	/* ==================== 辅助序列化类（可选） ==================== */
	public static class UdpSerializer
	{
		public static byte[] ToBytes(string text, int number, List<float> floats)
		{
			return LocalUdpClient.SerializeData(text, number, floats);
		}
	}
}